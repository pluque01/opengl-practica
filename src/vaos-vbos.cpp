...
void DescrVBOAtribs::crearVBO() {
  ...
  //
  // añadir el código para crear el VBO (transferir datos y registrar
  // metadatos), se deben de dar estos pasos:
  //
  // 1. generar un nuevo identificador o nombre de VBO
  glGenBuffers(1, &buffer);
  // 2. fija este buffer como buffer 'activo' actualmente en el 'target'
  // GL_ARRAY_BUFFER
  ...
  // 4. registrar, para este índice de atributo, la localización y el
  // formato de la tabla en el buffer
  glVertexAttribPointer(index, size, type, GL_FALSE, stride, offset);
  // 5. desactivar el buffer
  // 6. habilitar el uso de esta tabla de atributos
  glEnableVertexAttribArray(index); // <- 6
  glBindBuffer(GL_ARRAY_BUFFER, 0); // <- 5
...
void DescrVBOInds::crearVBO() {
  ...
  //
  // Añadir el código para crear el VBO en la GPU y transferir los datos, se
...
DescrVAO::DescrVAO(const unsigned p_num_atribs,
                   DescrVBOAtribs *p_dvbo_posiciones) {
  // comprobar precondiciones
  assert(0 < p_num_atribs);
  assert(p_dvbo_posiciones !=
         nullptr); // no se permite puntero nulo, debe apuntar a algún objeto
  assert(p_dvbo_posiciones->leerIndex() ==
...
void DescrVAO::crearVAO() {
  ...
  // 2. Para cada VBO de atributos adjunto al VAO (puntero en
  // 'dvbo_atributo' no nulo):
  ...
  // for (DescrVBOAtribs *i : dvbo_atributo) {
  //   if (i != nullptr) {
  //     i->crearVBO();
  //   }
  // }
  ...
  // 4. Para cada VBO de atributos adjunto al VAO (puntero en
  // 'dvbo_atributo' no nulo):
...
void DescrVAO::draw(const GLenum mode) {
  ...
  //
  // 1. Comprobar si el array se ha creado o no (antes de crearse, 'array' vale
  // 0, después es >0).
  ...
  //
  array == 0 ? crearVAO() : glBindVertexArray(array);

  // 2. Comprobar si la secuencia es indexada o no lo es (si no es indexada
  // 'dvbo_indices' es nulo)
  //    Si la secuencia es indexada
  //     - visualizar con 'glDrawElements'
  //    Si la secuencia no es indexada
  //     - visualizar con 'glDrawArrays'
  dvbo_indices == nullptr ? glDrawArrays(mode, first, count)
                          : glDrawElements(mode, idxs_count, idxs_type, offset);
  // INFO: PREGUNTAR POR QUÉ EL OFFSET
