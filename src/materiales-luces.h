...
public:
  ...
  // El nombre del archivo debe ir sin el 'path', se busca en 'materiales/imgs'
  // y si no está se busca en 'archivos-alumno'
...
protected:       //--------------------------------------------------------
  ...
  bool enviada = false;      // true si ha sido enviada, false en otro caso
  GLuint ident_textura = -1; // 'nombre' o identif. de textura para OpenGL
  unsigned ancho = 0,        // número de columnas de la imagen
      alto = 0;              // número de filas de la imagen
  unsigned char *imagen = nullptr; // pixels de la imagen, por filas.
  ModoGenCT modo_gen_ct =
      mgct_desactivada; // modo de generacion de coordenadas de textura
  float coefs_s[4] =
      {1.0, 0.0, 0.0,
       0.0}, // si 'modo_gen_ct != desactivadas', coeficientes para la coord. S
...
public:
  ...
  Material(/** const Tupla3f & p_color_ad,**/ const float p_k_amb,
           const float p_k_dif, const float p_k_pse, const float p_exp_pse);
...
protected:
  ...
  std::string nombre_mat = "material anónimo"; // nombre del material

  // Tupla3f
  //    color_ad = { 0.6f, 0.6f, 0.6f }; // color para componentes difusa y
  //    ambiental
  Textura *textura = nullptr; // si != nullptr, se usa color de la textura en
                              // lugar de 'color_ad'
...
public:
  float longi, // longitud actual de la fuente direccional (en grados, entre 0 y
               // 360)
      lati; // latitud actual de la fuente direccional (en grados, entre -90 y
            // 90)

protected:
  glm::vec3 color; // color de la fuente de luz (sirve para todas las
                   // componentes del MIL)
...
private:
  ...
  unsigned i_fuente_actual = 0; // fuente que se modifica al llamar a
                                // 'actualizar' (rota con pg.arriba y pg.abajo)
};
...
public:
  ...
  void
  pop(); // invoca 'activar' para el material en el tope y lo elimina del tope
