...
static unsigned x_ant_mabd; // coord. X anterior del ratón en modo arrastrar con
                            // botón derecho pulsado
static unsigned y_ant_mabd; // coord. Y anterior del ratón en modo arrastrar con
                            // botón derecho pulsado
...
void FGE_Scroll(GLFWwindow *window, double xoffset, double yoffset) {
  ...
  if (fabs(yoffset) <
      0.05) // si hay poco movimiento vertical, ignorar el evento
    return;

  constexpr float cam_d_z_incre_scroll = 1.0;
  const float direccion = 0.0 < yoffset ? +1.0 : -1.0;

  CamaraInteractiva *camara =
      aplicacionIG->escenas[aplicacionIG->ind_escena_act]->camaraActual();
...
void FGE_PulsarLevantarBotonRaton(GLFWwindow *window, int button, int action,
  ...
  // cout << "click, tam ventana == " << apl->ventana_tam_x << " x " <<
  // apl->ventana_tam_y << endl ; cout << "click, cursor pos  == " << x << ", "
  // << y <<  endl ;

  if (button == GLFW_MOUSE_BUTTON_LEFT) {
    // pulsar botón izquierdo: selección
    if (aplicacionIG->seleccion(
            x * aplicacionIG->mouse_pos_factor,
            (aplicacionIG->ventana_tam_y - y * aplicacionIG->mouse_pos_factor)))
      aplicacionIG->revisualizar_escena = true;
...
void FGE_MovimientoRaton(GLFWwindow *window, double xpos, double ypos) {
  ...
  // calcular el desplazamiento en pixels en X y en Y respecto de la última
  // posición
  const int x = int(xpos), y = int(ypos), dx = x - x_ant_mabd,
            dy = y - y_ant_mabd;
