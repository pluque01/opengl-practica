...
public:
  // desplazar o rotar la cámara en horizontal 'da' unidades y en vertical 'db'
  ...
  // (el eje es el eje Z, relativo al marco de la camara, es decir,
  // perpendicular al plano de visión)
  virtual void moverZ(const float dz) = 0;

  // hacer que apunte hacía el punto de atención 'paten' y
  // cambiar de modo a modo examinar
  // por defecto imprime mensaje de advertencia de que la cámra no ofrece esta
  // funcionalidad
  ...
  virtual std::string descripcion() { return "cámara interactiva"; }
};
...
protected:
  ...
  float a = 0.0, // ángulo horizontal (angulo del eje Z con el plano z==0 en
                 // cc.m.)
...
public:
  ...
  Camara3Modos(const bool perspectiva_ini, const glm::vec3 &origen_ini,
               const float ratio_vp_ini, const glm::vec3 &punto_aten_ini,
               const float fovy_grad_ini = 70.0);
...
private:
  ...
  // (0=examinar, 1=prim.persona con desplazamientos, 2=prim.persona con
  // rotaciones)
  ...
  // coordenadas polares del origen de coordenadas del mcv, respecto del punto
  // de atención las componentes 0 y 1 son los ángulos de longitud y latitud,
  // ambos EN RADIANES, inicialmente 0 la componente 2 es la distancia al
  // origen, inicialmente 'd_ini'
  glm::vec3 org_polares = {0.0, 0.0, d_ini};

  // coordenadas cartesianas del origen de coordenadas del mcv, respecto del
  // punto de atención ( == origen del MCV menos punto de atención)
