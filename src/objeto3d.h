...
public:
  ...
  // visualizar el objeto con OpenGL, usando la configuración especificada la
  // aplicación
  //
  virtual void visualizarGL() = 0;

  // visualizar pura y simplemente la geometría, sin colores, normales, coord.
  // text. etc... (se supone que el estado de OpenGL está fijado antes de esta
  // llamada de alguna forma adecuada)
  //
  virtual void visualizarGeomGL() = 0;

  // Visualizar las normales del objeto, si se puede hacer, en otro caso no hace
  // nada.
  //
  virtual void visualizarNormalesGL() = 0;

  // Visualizar el objeto en 'modo seleccion', es decir, sin iluminación y con
  // los colores basados en los identificadores de los objetos
  //
  virtual void visualizarModoSeleccionGL() = 0;

  // Ejecutar código después de producirse un click sobre este objeto.
  // Se debe redefinir para comportamientos específicos en clases derivadas de
  // 'Objeto3D'. Si no se redefine, la versión por defecto da estos pasos:
  ...
  // @return (bool) --  'true' si el estado del objeto ha cambiado y hay que
  // revisualizar, false en otro caso.
  //
  ...
  // @return (bool) 'true' si el objeto tiene asignado un color, 'false' en otro
  // caso
  //
  bool tieneColor();
  ...
  // (por defecto produce un error: número de parámetro fuera de rango, esto se
  // debe a que por defecto un objeto 3D declara tener 0 parámetros, y esta
  // función no se debe llamar, a no ser que se declaren tener más parámetros).
  // (virtual: redefinir en clases derivadas)
  //
  //  @param iParam (unsigned) -- índice del parámetro (entre 0 y
  //  numParametros()-1 )
  //  @param t_sec  (float)    -- tiempo en segundos desde el estado inicial
  //
  virtual void actualizarEstadoParametro(const unsigned iParam,
                                         const float t_sec);

  // ---------------------------------------------------------------------------------
  // métodos no virtuales, relativos a los parámetros, ya implementados en
  // 'Objeto3D'
  ...
  // (por defecto, para un objeto con 0 parámetros produce un error: número de
  // parámetro fuera de rango)
  //
  //  @param iParam (unsigned) -- índice del parámetro (entre 0 y
  //  numParametros()-1 )
  //  @param delta  (int)      --  número de unidades de incremento o decremento
  //  (típicamente +1 o -1)
  //
  ...
  //
  // @param dtSec (float) -- es el tiempo en segundos transcurrido desde el
  // inicio de las
  ...
  //    modelview:  matriz de modelado del padre del objeto (pasa coords.loc. a
  //    WC)
  //                (en el nodo raíz de un grafo de escena es la matriz
  //                identidad)
  //
...
private: //-----
  ...
  // (inicialmente está vacío, así todos los objetos tienen 0 parámetros por
  // defecto)
