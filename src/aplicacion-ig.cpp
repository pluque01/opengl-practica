...
// **
// ** Implementación de la clase 'AplicacionIG' y puntero a la aplicación
// actual.
// ** Copyright (C) 2016-2024 Carlos Ureña
// **
// ** Implementación de los métodos de la clase
// ** Puntero a la aplicación actual, inicialmente nulo (la instancia se crea en
// la func. 'main')
// **
...
void FGE_CambioTamano(GLFWwindow *ventana_glfw, int nuevo_ancho_fb,
                      int nuevo_alto_fb) {
...
// ---------------------------------------------------------------------
// inicialización de GLFW: creación de la ventana, designar FGEs. Se llama desde
// el constructor.
...
void AplicacionIG::inicializarGLFW() {
  ...
  glfwSetErrorCallback(
      ErrorGLFW); // fijar función llamada ante error (aborta, ver 'ig-aux.cpp')
  glfwInit();     // inicializacion de GLFW

  // Especificar versión de OpenGL y parámetros de compatibilidad que se querrán
  // (pedimos una version 'compatible' con OpenGL 3.3 'core', creo que eso
  // incluye versiones posteriores)
  ...
  ventana_glfw = glfwCreateWindow(wx, wy, "Practicas IG GIM+GIADE (24-25)",
                                  nullptr, nullptr); // crea ventana
  ...
  // Leer el tamaño real actual de la ventana (en 'ventana_tam_x'/
  // 'ventana_tam_y')
  // * Los eventos de ratón del gestor de ventanas usan el tamaño de la ventana.
  // * OpenGL usa el tamaño del framebuffer
  // En algunos casos ambos tamaños pueden ser distintos, así que a veces es
  // necesario usar un factor de conversión para las coordenadas de los eventos
  // de ratón.
  ...
  mouse_pos_factor =
      fbx > 0 ? fbx / wx : 1; // en displays macos retina, esto es 2
...
void AplicacionIG::visualizarFrame() {
  ...
  //    + dibujar los polígonos rellenos más atrás que las aristas (fijar el
  //    'polygon offset')
  ...
  // visualizar en pantalla el buffer trasero (donde se han dibujado las
  // primitivas)
...
void AplicacionIG::buclePrincipalGLFW() {
  ...
  while (!terminar_programa) {
    ...
    //
    // hay una animación en curso si están las animaciones activdas por el
    // usuario y además el objeto actual tiene al menos un parámetro animable:

    Objeto3D *objeto = escenas[ind_escena_act]->objetoActual();
    assert(objeto != nullptr);
    const bool animacion_activa =
        AnimacionesActivadas() && 0 < objeto->leerNumParametros();

    // 3. procesar eventos

    if (animacion_activa) // si hay alguna animación en curso
    {                     //
      glfwPollEvents(); // procesar todos los eventos pendientes, y llamar a la
                        // función correspondiente, si está definida
                        //
      if (!revisualizar_escena) // si no es necesario redibujar la ventana
        if (ActualizarEstado(*objeto)) //  actualiza estado, devuelve 'true' si
                                       //  'objeto' tiene parámetros animables
          revisualizar_escena =
              true; //  es necesario redibujar pq. el objeto se ha actualizado
    } //
    else                // si no hay una animacion en curso
      glfwWaitEvents(); //   esperar hasta que haya un evento y llamar a la
                        //   función correspondiente, si está definida

    // 4. determinar si se debe terminar el programa
    //
    // actualiza 'terminar_programa' si GLFW indica que se debe cerrar la
    // ventana
