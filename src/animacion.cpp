...
// (si lo están, se llama periodicamente a 'ActualizarEstado' para el objeto
// actual)
...
bool AnimacionesActivadas() { return animaciones_activadas; }

// --------------------------------------------------------------------------

bool ActualizarEstado(Objeto3D &objeto) {
  using namespace std;
  // cout << __FUNCTION__ << ", actualizando estado de objeto == " <<
  // objeto.leerNombre() << ", num pars == " << objeto.leerNumParametros() <<
  // endl ;
  //  si el objeto no tiene parámetros, no hay nada que hacer (no se puede
  //  animar)
...
bool ProcesarTeclaAnimacion(Objeto3D *objeto, int glfw_key) {
  ...
  case GLFW_KEY_RIGHT_BRACKET: // tecla '+' en el teclado normal (NO en la
                               // plantilla)
  case GLFW_KEY_KP_ADD:        // tecla '+' en el teclado numérico ¿?
    if (!animaciones_activadas) {
      animaciones_activadas = true;
      ultima_actu =
          steady_clock::now(); // registrar instante de última actualización
  ...
  case GLFW_KEY_SLASH: // tecla con '-' y '_' en el teclado normal (NO en la
                       // plantilla)
  ...
  case GLFW_KEY_DOWN:
    if (!animaciones_activadas)
      ...
      cout
          << "No se puede modificar un parámetro con las animaciones activadas."
          << endl;
      redisp = false;
    }
    break;
