...
// **
// ** Gestión de grafos de escena (implementación). Clase 'NodoGrafoEscena' y
// relacionadas.
...
// **     + Tipo enumerado 'TipoEntNGE' (tipo de entradas del nodo del grafo de
// escena)
// **
...
void NodoGrafoEscena::visualizarGL() {
  ...
  // pero teniendo en cuenta que, al igual que el método visualizarGL de las
  // mallas indexadas, si el nodo tiene un color, debemos de cambiar el color
  // del cauce (y hacer push/pop). Además, hay que hacer push/pop de la pila de
  // modelado. Así que debemos de dar estos pasos:
  //
  ...
  //     - fijar el color en el cauce (con 'fijarColor'), usando el color del
  //     objeto (se lee con 'leerColor()')
  // 2. Guardar copia de la matriz de modelado (con 'pushMM'),
  // 3. Para cada entrada del vector de entradas:
  //     - si la entrada es de tipo objeto: llamar recursivamente a
  //     'visualizarGL'
  //     - si la entrada es de tipo transformación: componer la matriz (con
  //     'compMM')
  ...
  // COMPLETAR: práctica 4: añadir gestión de los materiales cuando la
  // iluminación está activada
  //
  // Si 'apl->iluminacion' es 'true', se deben de gestionar los materiales:
  //
  //   1. al inicio, hacer 'push' de la pila de materiales (guarda material
  //   actual en la pila)
  //   2. si una entrada es de tipo material, activarlo usando a pila de
  //   materiales
...
// *****************************************************************************
// visualizar pura y simplemente la geometría, sin colores, normales, coord.
// text. etc...
...
void NodoGrafoEscena::visualizarGeomGL() {
  ...
  // COMPLETAR: práctica 3: implementar la visualización del nodo (ignorando
  // colores)
  //
  // Este método hace un recorrido de las entradas del nodo, parecido a
  // 'visualizarGL', pero más simple, Se dan estos pasos:
  //
...
void NodoGrafoEscena::visualizarNormalesGL() {
  ...
  //
  // Este método hace un recorrido de las entradas del nodo, parecido a
  // 'visualizarGL', teniendo en cuenta estos puntos:
  //
  // - usar push/pop de la matriz de modelado al inicio/fin (al igual que en
  // visualizatGL)
  // - recorrer las entradas, llamando recursivamente a 'visualizarNormalesGL'
  // en los nodos u objetos hijos
  // - ignorar el color o identificador del nodo (se supone que el color ya está
  // prefijado antes de la llamada)
...
void NodoGrafoEscena::visualizarModoSeleccionGL() {
  ...
  //
  // 2. Leer identificador (con 'leerIdentificador'), si el identificador no es
  // -1
  //      + Guardar una copia del color actual del cauce (con 'pushColor')
  //      + Fijar el color del cauce de acuerdo al identificador, (usar
  //      'ColorDesdeIdent').
  // 3. Guardar una copia de la matriz de modelado (con 'pushMM')
  // 4. Recorrer la lista de nodos y procesar las entradas transformación o
  // subobjeto:
  //      + Para las entradas subobjeto, invocar recursivamente a
  //      'visualizarModoSeleccionGL'
...
glm::mat4 *NodoGrafoEscena::leerPtrMatriz(unsigned indice) {
  // COMPLETAR: práctica 3: leer un puntero a una matriz en una entrada de un
  // nodo
  //
