...
// **          + Camara3Modos: cámara con tres modos de funcionamiento (práctica
// 5)
// **
...
void Camara::actualizarMatrices() {
  ...
  matriz_proye =
      scale(vec3{1.0f, 1.0f / ratio_vp,
                 1.0f}); // MAT_Escalado( 1.0f, 1.0f/ratio_vp, 1.0f );
  matrices_actualizadas = true;
}
// -----------------------------------------------------------------------------
// lee la descripción de la cámara (y probablemente su estado)

std::string Camara::descripcion() { return "cámara (clase base)"; }
...
// ----------------------------------------------------------------------------
// hacer que apunte hacía el punto de atención 'paten' y cambiar de modo a modo
// examinar por defecto imprime mensaje de advertencia de que la cámra no ofrece
// esta funcionalidad
...
// por defecto imprime mensaje de advertencia de que no ofrece esta
// funcionalidad

void CamaraInteractiva::siguienteModo() {
  using namespace std;
  cout << "esta cámara no tiene definidos varios modos de funcionamiento."
       << endl;
}
...
void CamaraOrbitalSimple::moverZ(const float dz) {
  constexpr float d_min = 0.2, // distancia mínima al origen
...
void CamaraOrbitalSimple::actualizarMatrices() {
  using namespace glm;
  matriz_vista =
      translate(vec3(0.0, 0.0, -d)) * // MAT_Traslacion( { 0.0, 0.0, -d } ) *
      rotate(radians(b),
             vec3(1.0, 0.0, 0.0)) * // MAT_Rotacion( b,  { 1.0,0.0,0.0} ) *
      rotate(radians(-a),
             vec3(0.0, 1.0, 0.0)); // MAT_Rotacion( -a, { 0.0,1.0,0.0}  ) ;

  constexpr float fovy_grad = 70.0, near = 0.05, far = near + 1000.0;

  // MAT_Perspectiva( fovy_grad, ratio_vp, near, far ); // CUA: ratio_vp es y/x,
  // pero esta función esperael 'aspect', que parece ser x/y

  matriz_proye =
      perspective(radians(fovy_grad), 1.0f / ratio_vp, near, far); // CUA
...
std::string CamaraOrbitalSimple::descripcion() {
  using namespace std;
  return string("cámara órbital simple, ángulos: a = ") + to_string(a) +
         ", b = " + to_string(b);
}
...
static const std::string nombre_modo[3] = {
    "examinar (o modo orbital)", "primera persona con rotaciones",
    "primera persona con desplazamientos"};

// ----------------------------------------------------------------------------
// convierte unas coordenadas esfericas (a,b,r) a cartesianas (x,y,z)
// ('a' es el ángulo de rotacion en radianes respecto del eje Z, en el plano
// Y=0)

glm::vec3 Cartesianas(const glm::vec3 &esfericas) {
  const float sa = std::sin(esfericas[0]), ca = std::cos(esfericas[0]),
...
glm::vec3 Esfericas(const glm::vec3 &cartesianas) {
  const float x = cartesianas[0], y = cartesianas[1], z = cartesianas[2],
              r = std::sqrt(x * x + y * y +
                            z * z),  // longitud del vector  (radio)
...
void TestCartesianasPolares() {
  ...
  for (unsigned long i = 0; i < 1024 * 1024; i++) {
    ...
    const glm::vec3 cart = {2.0f * x - 1.0f, 2.0f * y - 1.0f, 2.0f * z - 1.0f},
                    pol = Esfericas(cart), cart2 = Cartesianas(pol);
    const float lsq = length2(cart2 - cart);
    if (lsq > max)
      max = lsq;
  }
...
//
// * perspectiva_ini : true si es una camara perspectiva, false si es
// ortográfica
...
// * fovy_grad_ini   : si es perspectiva, la apertura de campo vertical, en
// grados

Camara3Modos::Camara3Modos(const bool perspectiva_ini,
                           const glm::vec3 &origen_ini,
                           const float ratio_vp_ini,
                           const glm::vec3 &punto_aten_ini,
                           const float fovy_grad_ini) {
  // using namespace std ;
...
// ----------------------------------------------------------------------------
// desplazar o rotar la cámara en horizontal 'dx' unidades y en vertical 'dy'
// unidades (en horizontal y en vertical aquí se entiende relativo al marco de
// camara)

void Camara3Modos::desplRotarXY(const float da, const float db) {
  float da_p = da / 100.0, db_p = db / 100.0;
  switch (modo_actual) {
  case ModoCam::examinar: {
    // COMPLETAR: práctica 5: rotar la cámara en modo examinar
    //
    // actualizar las dos primeras componentes (ángulos) de las coordenadas
    org_polares += glm::vec3(da_p, db_p, 0.0f);

    // polares actualizar las coordenadas cartesianas a partir de las polares
  ...
  case ModoCam::prim_pers_rot: {
    ...
    //
    // 1. actualizar las dos primeras componentes (ángulos) de las coordenadas
    // polares (igual que en el modo examinar)
    org_polares += glm::vec3(da_p, db_p, 0.0f);

    // 2. calcular las nuevas coordenadas cartesianas, y el vector de
    // desplazamiento desde las nuevas a las antiguas
    glm::vec3 new_org_carts = Cartesianas(org_polares);
    glm::vec3 v_despl = new_org_carts - org_cartesianas;

    // 3. restarle al punto de atención ese vector de desplazamiento
    punto_atencion -= v_despl;

    // 4. actualizar las coordenadas cartesianas
    org_cartesianas = new_org_carts;
  ...
  case ModoCam::prim_pers_despl: {
    // COMPLETAR: práctica 5: desplazar la cámara en modo 1a persona con
    // desplazamientos
    //
    ...
    punto_atencion += eje[X] * da_p + eje[Y] * db_p;
...
// (el eje es el eje Z, relativo al marco de la camara, es decir, perpendicular
// al plano de visión)
...
void Camara3Modos::moverZ(const float dz) {
  ...
  case ModoCam::examinar: {
    ...
    // .....
    constexpr float ratio_crecimiento = 0.02;
    constexpr float r_min = 0.2;

    org_polares[2] = r_min + (org_polares[2] - r_min) *
                                 std::pow((1.0f + ratio_crecimiento), dz);
    org_cartesianas = Cartesianas(org_polares);
...
// ----------------------------------------------------------------------------
// hacer que apunte hacía el punto de atención 'nuevo_punto_aten' y pasar a modo
// examinar
...
void Camara3Modos::siguienteModo() {
  ...
  using namespace std;
  cout << "Modo de cámara cambiado a: " << nombre_modo[int(modo_actual)] << "."
       << endl;
  matrices_actualizadas = false;
}
// ----------------------------------------------------------------------------
...
void Camara3Modos::actualizarMatrices() {
  ...
  if (perspectiva)
    matriz_proye = perspective(radians(fovy_grad), 1.0f / ratio_vp, near,
                               far); // CUA: ratio_vp es y/x, pero esta función
                                     // espera el 'aspect', que parece ser x/y
  else {
    constexpr float s = 8.0;
    matriz_proye = ortho(
        -s / 2.0f, +s / 2.0f, -s * ratio_vp / 2.0f, +s * ratio_vp / 2.0f,
        -20.0f, 100.0f); // MAT_Ortografica( -s/2.0f, +s/2.0f, -s*ratio_vp/2.0f,
                         // +s*ratio_vp/2.0f, -20.0f, 100.0f );
  }
  matrices_actualizadas = true;
}
// ----------------------------------------------------------------------------

glm::vec3 Camara3Modos::puntoAtencion() { return punto_atencion; }
// -----------------------------------------------------------------------------
// lee la descripción de la cámara (y probablemente su estado)
