...
// **
// ** Gestión de eventos de teclado (implementación). Función
...
void FGE_PulsarLevantarTecla(GLFWwindow *window, int key, int scancode,
  ...
  if (action ==
      GLFW_PRESS) // solo estamos interesados en el evento de levantar una tecla
  ...
  // si está pulsada la tecla 'L', actualizar la colección de fuentes de luz y
  // terminar
  if (glfwGetKey(window, GLFW_KEY_L) == GLFW_PRESS) {
    // COMPLETAR: Práctica 4: procesar la tecla 'key' para actualizar la
    // colección de fuentes de luz actual (usar método 'colFuentes' de la escena
    // activa para obtener un puntero), llamar a 'ProcesaTeclaFuenteLuz', si
    // devuelve 'true', forzar revisualizar escena (asignar 'true' a
    // 'apl->revisualizar_escena')
    // .....
    aplicacionIG->revisualizar_escena =
        ProcesaTeclaFuenteLuz(escena->colFuentes(), key);
    return; // finalizar la f.g.e, ya que si está la tecla L pulsada no se mira
            // ninguna otra tecla.
  }
  ...
  if (glfwGetKey(window, GLFW_KEY_A) == GLFW_PRESS) {
    ...
    //
    // procesar la tecla en la variable 'key' para actualizar el estado de
  ...
  constexpr float
      cam_d_incre_tecla =
          1.0, // incremento de 'cam_d' por teclado (debe ser >= 1.0)
  ...
  case GLFW_KEY_P:
    ...
    aplicacionIG->ind_escena_act =
        (aplicacionIG->ind_escena_act + 1) % aplicacionIG->escenas.size();
    cout << "Escena actual cambiada a: " << (aplicacionIG->ind_escena_act + 1)
         << endl
         << flush;
    break;
  ...
  case GLFW_KEY_E:
    ...
    cout << "Dibujar ejes: "
         << (aplicacionIG->dibujar_ejes ? "activado." : "desactivado.") << endl
         << flush;
    break;

  case GLFW_KEY_N:
    aplicacionIG->visualizar_normales = !aplicacionIG->visualizar_normales;
    cout << "Visualizar normales: "
         << (aplicacionIG->visualizar_normales ? "activado." : "desactivado.")
         << endl
         << flush;
    break;

  case GLFW_KEY_M:
    aplicacionIG->modo_visu = ModosVisu((int(aplicacionIG->modo_visu) + 1) %
                                        int(ModosVisu::num_modos));
    cout << "Modo de visualización cambiado a: '"
         << nombreModoVisu[int(aplicacionIG->modo_visu)] << "'" << endl
         << flush;
    break;

  case GLFW_KEY_W:
    aplicacionIG->dibujar_aristas = !aplicacionIG->dibujar_aristas;
    cout << "Dibujar aristas: "
         << (aplicacionIG->dibujar_aristas ? "activado" : "desactivado")
         << endl;
    break;
  ...
  case GLFW_KEY_I:
    ...
    cout << "Iluminación : "
         << (aplicacionIG->iluminacion ? "activada" : "desactivada") << endl
         << flush;
    break;

  case GLFW_KEY_F:
    aplicacionIG->usar_normales_tri = !aplicacionIG->usar_normales_tri;
    cout << "La iluminación usa "
         << (aplicacionIG->usar_normales_tri
                 ? "la normal del triángulo"
                 : "las normales de vértices (interpoladas)")
         << endl
         << flush;
    break;
  ...
  case GLFW_KEY_G:
    ...
    cout << "suavizar_aristas : "
         << (aplicacionIG->suavizar_aristas ? "activado" : "desactivado")
         << endl
         << flush;
    break;
    ...
    //
    // Añadir un caso nuevo: si se ha pulsado la tecla 'Y' (GLFW_KEY_Y),
  ...
  case GLFW_KEY_T:
    ...
    cout << "imprimir tiempos : "
         << (aplicacionIG->imprimir_tiempos ? "activado" : "desactivado")
         << endl
         << flush;
    break;
