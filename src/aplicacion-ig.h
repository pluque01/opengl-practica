...
// **     + variables de estado para gestionar el bucle de eventos
// (revisualizar_escena, terminar_programa)
// **     + variables de estado del ratón para gestionar eventos de mover con
// botón pulsado (arrastrar).
// **     + diversas variables de estado que determinan como se visualizan los
// objetos, las animaciones, etc ....
// **
...
// declaraciones adelantadas de clases (para poder declarar punteros antes de
...
const std::string nombreModoVisu[int(ModosVisu::num_modos) + 1] = {
    "relleno", "líneas", "puntos", "*** NUM. MODOS. VIS. ***"};

// puntero al objeto de la clase 'AplicacionIG' con variables y parámetros
// globales
...
public:
  ...
  // Ejecuta el bucle principal  de gestion de eventos GLFW, hasta que se cierra
  // ventana o bien 'terminar_programa' se pone a 'true' en algún gestor de
  // eventos.
  void buclePrincipalGLFW();

  // Visualiza la escena en la ventana GLFW, se llama durante el bucle de
  // de gestión de eventos, en cada iteración, si hay que refrescar los
  // contenidos de la ventana
  void visualizarFrame();

  // Función principal de selección, se llama al hacer click con el botón
  // izquierdo usa la posición donde se ha hecho click en coordenadas del
  // dispositivo del FBO (enteras). Si en ese pixel hay un objeto con
  // identificador >0, ejecuta 'cuandoClick' del objeto. y pone la camara actual
  // de la escena actual mirando hacia el centro del objeto.
  //
  ...
  bool revisualizar_escena =
      true; // true indica que hay que redibujar la escena
  bool terminar_programa =
      false; // true indica que hay que cerrar la aplicación
  bool imprimir_tiempos =
      false; // 'true' imprimir tiempo por frame, 'false', no imprimir
  ...
  // true -> dibujando en modo selección con FBO invisible (puede ser el
  // back-buffer)
  ...
  // Indica si queremos visualizar normales al visualizar los objetos (true) o
  // no.
  bool visualizar_normales = false;

  // true cuando se está en proceso de visualizar normales durante una llamada a
