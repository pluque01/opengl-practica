...
Escena::Escena() {
  // COMPLETAR: práctica 4: inicializar la colección de fuentes y el material
  // inicial
  //
  // Se debe dar un valor inicial adecuado a las variables de instancia
  // 'col_fuentes' y 'material_ini'
  //
  ...
  //
  // Añadir sentencias 'push_back' para añadir varias cámaras al vector
  // 'camaras'. Eliminar este 'push_back' de la cámara orbital simple
  // ('CamaraOrbitalSimple') por varias cámaras de 3 modos ('Camara3Modos')
...
// especificada en 'cv' (pone 'apl->modo_selecion' a 'false' y queda así)
...
void Escena::visualizarGL() {
  ...
  const float ratio_vp =
      float(aplicacionIG->ventana_tam_y) / float(aplicacionIG->ventana_tam_x);

  // cout << "Escena::visualizarGL: dimen " << apl->ventana_tam_x << " x " <<
  // apl->ventana_tam_y << ", y/x == " << ratio_vp << endl ;

  camara->fijarRatioViewport(ratio_vp);
  // cout << endl << __FUNCTION__ << ": índice cámara actual == " <<
  // ind_camara_actual << endl ;
  ...
  // (es blanco debido a que el fondo se rellena con un color oscuro y debe
  // contrastar).
  ...
  // Visualizar las aristas del objeto, si procede (es decir: en modo
  // relleno, con visualización de aristas activada)

  if (aplicacionIG->dibujar_aristas &&
      aplicacionIG->modo_visu == ModosVisu::relleno) {
    // desactivar iluminación y texturas (podrían estarlo a partir de prác. 4)
    cauce->fijarEvalMIL(false);
    cauce->fijarEvalText(false);

    // COMPLETAR: práctica 1: visualizar únicamente las aristas del objeto
    // actual
    //
    // - hay que invocar 'visualizarGeomGL' para el objeto actual de la escena
    // ('objeto')
...
void Escena::visualizarGL_Seleccion() {
  // Comprobar algunas precondiciones y recuperar el cauce (para acortar la
  // anotación)
  assert(aplicacionIG != nullptr);
  assert(aplicacionIG->cauce != nullptr);
  Cauce *cauce = aplicacionIG->cauce;
  ...
  // COMPLETAR: práctica 5: visualizar el objeto raiz de esta escena en modo
  // selección
  //
  // Se deben dar los siguientes pasos:

  // (1) Configurar estado de OpenGL:
  //       + fijar el viewport (con 'glViewport') usando el tamaño de la ventana
  //       (guardado en 'apl'),
  ...
  //      + Poner el color actual del cauce a '0' (por defecto los objetos no
  //      son seleccionables)
  // ........
  cauce->activar();
  cauce->fijarEvalMIL(false);
  ...
  // (3) Limpiar el framebuffer (color y profundidad) con color (0,0,0) (para
  // indicar que en ningún pixel hay nada seleccionable)
  // ........
  // glClearColor(0.0, 0.0, 0.0, 1.0);
  glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);

  // (4) Recuperar la cámara actual (con 'camaraActual') y activarla en el
  // cauce,
  // ........
...
void Escena::siguienteCamara() {
  ...
  using namespace std;
  cout << "Cámara actual cambiada a: " << (ind_camara_actual + 1) << "/"
       << camaras.size() << ": " << camaras[ind_camara_actual]->descripcion()
       << endl;
}
